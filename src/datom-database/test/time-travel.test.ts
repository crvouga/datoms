...
describe.each(FIXTURES)("DatomDatabase (%s)", (_name, createFixture) => {
  ...
  describe("Time-Traveling Queries", () => {
    ...
    test("should support since queries in datalog", async () => {
      ...
      await db.transact([{ op: "add", e: 4, a: "name", v: "David" }]);
    ...
    test("should handle time-travel queries with date values", async () => {
      ...
      await db.transact([{ op: "add", e: 1, a: "created", v: date3 }]);
    ...
    test("should handle time-travel queries with reference values", async () => {
      ...
      await db.transact([{ op: "add", e: 1, a: "parent", v: 20 }]);
      await db.transact([{ op: "add", e: 2, a: "parent", v: 10 }]);
    ...
    test("should handle complex since query scenario", async () => {
      ...
      await db.transact([{ op: "sub", e: 1, a: "status", v: "completed" }]);
      await db.transact([{ op: "add", e: 1, a: "status", v: "failed" }]);
